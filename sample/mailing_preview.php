...
echo "<h1>Envoi d’un Mailing en 2 étapes</h1>";
...
try
{
    ...
    echo "Votre mailing est transmis sur la plate-forme et porte l'ID : <b>", $monMailing->jobID, "</b><br />";
    $nb = $mailingPreviewResult->spLetterssCount;
    echo "Votre mailing contient <b>{$nb} lettres</b> ";
    
    if ($mailingPreviewResult->spExpectedDispatchNotice == 0)
        echo "et pourrait être produit <b>aujourd’hui</b> <br />" ;
    else
        echo "et pourrait être produit dans <b>", $mailingPreviewResult->spExpectedDispatchNotice , " jour(s)</b><br />" ;
...
{
    die("Erreur lors de la préparation du mailing de Service Postal : ".$e->getMessage() );
}
...
echo "<a class='btn btn-success' href='mailing_validate.php?servicePostalID=",$monMailing->jobID,"'><span class='glyphicon glyphicon-ok'></span> Valider ce mailing</a> ";
